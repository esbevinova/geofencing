...
app.post("/childFCMTokenUpdate", async (req, res) =>{
  try{
    ...
    else if(foundChild.fcmToken == childFcmToken){
      res.send("the token is the same")
    }
  } catch (e){
    console.log(e)
...
app.post("/saveGeofenceEventToServer", async (req, res) => {
  try{
    ...
    {
      var child_id = parsedJson[i].childId
      var geofence_id = parsedJson[i].geofenceId
      var latitude = parsedJson[i].latitude
      var longtitude = parsedJson[i].longitude
      var accuracy = parsedJson[i].accuracy
      var speed = parsedJson[i].speed
      var altitude = parsedJson[i].altitude
...
app.post("/returnAlertHistory", async (req, res) => {
  try{
    ...
    const parentIdFound = childFound.parentId //get parent Id from the found child
    console.log(receivedChildId)
...
app.post("/allChildren", async (req, res) => {
  try{
    receivedParentId = req.body.parentId
    receivedFcmToken = req.body.fcmToken
    const foundParent= await users.get(receivedParentId)
    var existingFCMToken = foundParent.fcmToken
    parsedId = ObjectId(foundParent._id)
    if (receivedFcmToken == existingFCMToken){
      allChildren = await users.returnAllChildren(parsedId)
